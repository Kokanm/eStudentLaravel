...
class IzbirniPredmetiController extends Controller {
	...
	public function izberi($vp){
        ...
        if(array_key_exists('strokovni', $list)) {
            if ($list['prosti'] == $list['strokovni'] && $list['prosti'] != 0) {
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni' => [], 'tips' => 1])->withErrors("Izberite različne prosto izbirne predmete!");
            }
            ...
            for($i=1; $i<=6; $i++){
                if ($modularni[$list['modularni' . $i]] == $prosti[$list['prosti']] && $list['modularni' . $i] != 0) {
                    return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                        'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>$modularni, 'tips'=>1])->
            ...
            if(in_array(Predmet::where('naziv_predmeta', explode(" - ", $prosti[$list['prosti']])[0])->pluck('sifra_predmeta'), $mod1)){
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>[], 'tips'=>1])->
                withErrors("Izberite različne modularne predmete! ");
            }
        ...
        if(array_key_exists('strokovni', $list)){
            if($list['strokovni'] != 0) {
                ...
                return view('predmeti', ['studijski_program' => $stp, 'predmeti'=>$obvezni_predmeti, 'sum' => $sum,
                    'prosti'=>$prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni' => [], 'tips'=>0])->withErrors("Izberite en strokovni predmet!");
            }
        }
        ...
        if($suma < 60) {
            if($vpisna['sifra_letnika']==3 && $vpisna['zeton'] == 1) {
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>$modularni, 'tips'=>0])->withErrors("Nimate dovolj KT!");
            }else{
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>[], 'tips'=>0])->withErrors("Nimate dovolj KT!");
            }
        }
    ...
    public function izberi2($vp){
        ...
        $strokovni = [];
        for ($i=0; $i < count($strokovno_izbirni); $i++){
            $strokovni[$i] = $strokovno_izbirni[$i]." ".Predmet::where('sifra_predmeta', $strokovno_izbirni[$i])->pluck('naziv_predmeta'). " - ".Predmet::where('sifra_predmeta', $strokovno_izbirni[$i])->
                pluck('stevilo_KT')." KT";
        }
        if(!empty($strokovni))
            array_unshift($strokovni, "");
        ...
        $modularni = [];
        for($i=0; $i<count($modpredmeti); $i++) {
            $modularni[$i] = $modpredmeti[$i]." ".Predmet::where('sifra_predmeta', $modpredmeti[$i])->pluck('naziv_predmeta'). " - ".Predmet::where('sifra_predmeta', $modpredmeti[$i])->
                pluck('stevilo_KT')." KT";
        }
        if(!empty($modularni))
            array_unshift($modularni, "");
        ...
        if($vpisna['sifra_letnika'] == 2) {
            $prosto_izbirni = Predmet_studijskega_programa::where('sifra_studijskega_programa', $vpisna['sifra_studijskega_programa'])->
            where('sifra_letnika', $vpisna['sifra_letnika'])->where('sifra_sestavnega_dela', '7')->orWhere('sifra_sestavnega_dela', '6')->lists('sifra_predmeta');
            $prosti = [];
            for ($i = 0; $i < count($prosto_izbirni); $i++) {
            ...
            $j = 0;
            for ($i = 0; $i < count($prosto_izbirni); $i++) {
                $prosti[$j] = $prosto_izbirni[$i]." ".Predmet::where('sifra_predmeta', $prosto_izbirni[$i])->pluck('naziv_predmeta') . " - " . Predmet::where('sifra_predmeta', $prosto_izbirni[$i])->
                    pluck('stevilo_KT') . " KT";
                $j++;
            }
            if (!empty($prosti))
        ...
        if(array_key_exists('prosti', $list) && array_key_exists('prosti2', $list)) {
            if ($list['prosti'] == $list['prosti2'] && $list['prosti'] != 0) {
                if ($vpisna['sifra_letnika'] == 3 && $vpisna['zeton'] == 1) {
                    return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                        'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni' => $modularni, 'tips' => 1])->withErrors("Izberite različne prosto izbirne predmete!");
                } else {
                    return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                        'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni' => [], 'tips' => 1])->withErrors("Izberite različne prosto izbirne predmete!");
                }
        ...
        if(array_key_exists('strokovni', $list)) {
            ...
            if(in_array(Predmet::where('naziv_predmeta', explode(" ", $prosti[$list['prosti']])[1])->pluck('sifra_predmeta'), $mod1)){
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>[], 'tips'=>1])->
                withErrors("Izberite različne modularne predmete! ");
            }

            if(in_array(Predmet::where('naziv_predmeta', explode(" ", $prosti[$list['prosti']])[1])->pluck('sifra_predmeta'), $mod2)){
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>[], 'tips'=>1])->
                withErrors("Izberite različne modularne predmete! ");
            }

            if(in_array(Predmet::where('naziv_predmeta', explode(" ", $prosti[$list['prosti2']])[1])->pluck('sifra_predmeta'), $mod1)){
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>[], 'tips'=>1])->
                withErrors("Izberite različne modularne predmete! ");
            }

            if(in_array(Predmet::where('naziv_predmeta', explode(" ", $prosti[$list['prosti2']])[1])->pluck('sifra_predmeta'), $mod2)){
                return view('predmeti', ['studijski_program' => $stp, 'predmeti' => $obvezni_predmeti, 'sum' => $sum,
                    'prosti' => $prosti, 'strokovni' => $strokovni, 'moduli' => $moduli, 'vpisna' => $vp, 'modularni'=>[], 'tips'=>1])->
                withErrors("Izberite različne modularne predmete! ");
            }
